<script lang="ts">
    ...
    import { ApiError, createReplay, fetchReplays } from "../../api";
    ...
    let latestRecord: ReplayRecord | null = null;

    const refresh = async () => {
        try {
            records = await fetchReplays();
        } catch (error) {
            errorMessage =
    ...
    const submit = async () => {
        const rawPayloadId = Number(form.rawPayloadId.trim());
        if (!Number.isInteger(rawPayloadId) || rawPayloadId < 1) {
            latestRecord = null;
            errorMessage = "Raw Payload ID must be a positive integer.";
            return;
        }

        try {
            latestRecord = await createReplay({
...
<div class="surface">
    ...
    <button type="button" onclick={submit}>Create Replay</button>
        ...
        <div class="list">
            {#each records as record}
                <div class="row">
                    ...
                    <span>{record.symbol} / {record.restore_status}</span>
                </div>
            {/each}
        </div>
    {/if}
