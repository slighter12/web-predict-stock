...
import backend.services.foundation_service as foundation_service
from backend.database import Base, DailyOHLCV, SymbolLifecycleRecord, TwCompanyProfile
from backend.errors import DataNotFoundError, UnsupportedConfigurationError
from backend.schemas.foundations import (
...
def test_dispatch_run_execution_route_requires_explicit_manual_confirmation(
    ...
    assert captured_profiles == [("live_ctrl_v1", "TW", foundation_service.LIVE_CONTROL_VERSION)]
...
def test_persist_run_peer_outputs_uses_market_scoped_snapshot(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda run_payload, overlays: captured.update(
            {"run_payload": run_payload, "overlays": overlays}
        )
        or {"id": 10},
    )
