...
| `OPEN-007` | `TBD-004` | Cross-model missing-feature default policy | model governance | Widening model families beyond ADR-0008 | A shared default behavior is defined, or it is explicitly decided that no shared default will exist |
...
| `OPEN-010` | — | Point-in-time universe status history | data platform | Any absolute historical performance claim; the Fundamental Research Track in ADR-0020 | `tw_company_profiles` carries validity ranges or a status event log, so past-date active membership is answerable for the post-ingestion window; the pre-ingestion gap in ADR-0012 is separately acknowledged as unrecoverable |
| `OPEN-011` | — | Reconciliation coverage-ratio threshold | data platform | ADR-0004 compliance for the guard in ADR-0023 | The literal coverage ratio guarding internal company-profile reconciliation is either labelled provisional or replaced by a named, versioned policy |
